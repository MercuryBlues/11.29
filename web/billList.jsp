		...
		<div class="content">
			<table class="list">
				<tbody>
					...
					<tr>
						...
						<td><span class="STYLE1">  <c:if test="${book.bookId==null}">
							否
						</c:if>
						...
						<td collapse=2>
							<a href="/deleteBook.action?name=${searchName}&id=${book.id}"><em>  <button type="button" class="layui-btn layui-btn-danger"><i class="layui-icon"></i></button></em></a>
							<a href="/selectById.action?id=${book.id}"><em> <button type="button" class="layui-btn"><i class="layui-icon"></i></button></em></a>
						</td>
			...
			<div align="center">当前页是第<span>${pageInfo.pageNum}</span>页&nbsp; 一共<span>${pageInfo.pages}</span>页
				<c:if test="${pageInfo.isFirstPage==false}">
					<a href="/BookList.action?page=${pageInfo.prePage}" > <button type="button" class="layui-btn layui-btn-primary layui-btn-sm"><i class="layui-icon"></i></button></a></c:if>

				<c:if test="${pageInfo.isFirstPage==true}">  <button type="button" class="layui-btn layui-btn-primary layui-btn-sm"><i class="layui-icon"></i></button></c:if>
				<c:if test="${pageInfo.isLastPage==false}">
					<a href="/BookList.action?page=${pageInfo.nextPage}"><button type="button" class="layui-btn layui-btn-primary layui-btn-sm"><i class="layui-icon"></i></button>
					</a></c:if>

				<c:if test="${pageInfo.isLastPage==true}"><button type="button" class="layui-btn layui-btn-primary layui-btn-sm"><i class="layui-icon"></i></button>
				</c:if>
			</div>
		</div>
	</div>
